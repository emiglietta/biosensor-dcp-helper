...
measurement <- tbl(pool.v3, "morphology_measurement")
existing_measurement <- measurement %>% dplyr::select(id_observation_checksum) %>% distinct() %>% collect()

new_measurement = tibble(barcode = measurement_id %>% str_extract(pattern = "0000\\d+"),
  ...
  select(-(t1:t3)) %>%
    ...
    mutate(id_observation_checksum = compute_observations_checksum(local_path %>% as.character()) %>% as.character()) %>%
...
new_measurement %>%
    dbWriteTable(pool.v3, "morphology_measurement", ., append = TRUE)
toc()
...
glimpse(new_measurement_2)

furrr::future_map2(new_measurement_2$observation,
                   new_measurement_2$local_path,
                     ~ { read_and_merge_observations(.y) %>%
                           mutate(observation = .x) %>%
                           separate(observation, c("id_observation", "id_observation_checksum"), sep = "___" ) %>%
                           dbWriteTable(pool.v3, "morphology_observation", ., append = TRUE)
                         # print(paste0("appended ", .y))
                         # flush.console()
                       }
    )
