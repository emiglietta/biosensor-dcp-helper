...
### Summary 'measurement' and 'observation' 
...
barcode_id <- "000012118203"

experiment_object_data <-  measurement %>% 
  filter(id_barcode==barcode_id) %>% 
  select(id_observation, measurement_no, iteration_no, well, field) %>% 
  merge(observation %>% select(id_observation, image_number, object_number), by="id_observation" ) %>% 
  distinct() %>% 
  collect()
...
```{r}
# letter2number <- function(x) { return (utf8ToInt(x) - utf8ToInt("A") + 1L)}

experiment_object_data_v2 <- experiment_object_data %>% 
  mutate(column=as.numeric(substr(well,2,3))) %>% 
  mutate(row=substr(well,1,1))

# experiment_object_data_v2$column <- (substr(experiment_object_data$well,2,3))
# experiment_object_data_v2$temprow <- (substr(experiment_object_data$well,1,1))
# # experiment_object_data_v2$row
# x <- lapply( experiment_object_data$temprow, FUN=letter2number )
```

```{r}
g <- experiment_object_data_v2 %>% 
  group_by(measurement_no, iteration_no, row, column) %>%
  summarize(n=n()) %>%
  ggplot( aes(x=column,y=row,fill=n) ) +
    ...
    ggtitle(paste0("Object count"), subtitle = barcode_id)

t <- Sys.time()
ggsave(paste0(barcode_id, "_", t, "_object_count.pdf"), plot=g, path="~/mcsaba/biosensor/logs/", dpi=300, bg="white")
```
...
```{r eval=FALSE, include=FALSE}
# tic()
# deleteQuery <- glue::glue_sql("DELETE FROM measurement WHERE id_measurement LIKE '%000012118303__2023-06-02T13_16_13-Measurement_1%'", .con=pool.v3)
# dbExecute(statement = deleteQuery, con=pool.v3)
# toc()
# tic()
# deleteQuery <- glue::glue_sql("DELETE FROM observation WHERE id_observation LIKE '%000012118303__2023-06-02T13_16_13-Measurement_1%'", .con=pool.v3)
# dbExecute(statement = deleteQuery, con=pool.v3)
# toc()
```
...
```
```{r}
v3.measurements <- unique(summary_observations$id_measurement)
v3ds.measurements <- unique(summary.observations.areashape.downsampling$id_measurement)


# idx <- (v3ds.measurements %in% v3.measurements)
# missing.measurements <- v3.measurements[!(v3ds.measurements %in% v3.measurements)]

missing.measurements <- setdiff(v3.measurements, v3ds.measurements)

print(missing.measurements)
```

## TODO - align measurement and observation tables
...
## Compare v3 and v3.downsampling
...
observation.downsampling.ch2 <- tbl(pool.v3.ds, "observation_ch2")

summary.observations.ch2.downsampling <- measurement.downsampling %>% 
  select(id_barcode, id_measurement, id_observation) %>% 
  left_join(observation.downsampling.ch2 %>% select(id_observation)) %>% 
  group_by(id_barcode, id_measurement) %>% 
  count() %>% 
  collect()

observation.downsampling.ch3 <- tbl(pool.v3.ds, "observation_ch3")

summary.observations.ch3.downsampling <- measurement.downsampling %>% 
  select(id_barcode, id_measurement, id_observation) %>% 
  left_join(observation.downsampling.ch3 %>% select(id_observation)) %>% 
  group_by(id_barcode, id_measurement) %>% 
  count() %>% 
  collect()

observation.downsampling.ch4 <- tbl(pool.v3.ds, "observation_ch4")

summary.observations.ch4.downsampling <- measurement.downsampling %>% 
  select(id_barcode, id_measurement, id_observation) %>% 
  left_join(observation.downsampling.ch4 %>% select(id_observation)) %>% 
  group_by(id_barcode, id_measurement) %>% 
  count() %>% 
  collect()

observation.downsampling.ch5 <- tbl(pool.v3.ds, "observation_ch5")

summary.observations.ch5.downsampling <- measurement.downsampling %>% 
  select(id_barcode, id_measurement, id_observation) %>% 
  left_join(observation.downsampling.ch5 %>% select(id_observation)) %>% 
  group_by(id_barcode, id_measurement) %>% 
  count() %>% 
  collect()

observation.downsampling.ch6 <- tbl(pool.v3.ds, "observation_ch6")

summary.observations.ch6.downsampling <- measurement.downsampling %>% 
  select(id_barcode, id_measurement, id_observation) %>% 
  left_join(observation.downsampling.ch6 %>% select(id_observation)) %>% 
  group_by(id_barcode, id_measurement) %>% 
  count() %>% 
  collect()
...
merged_summary_of_measurements <- summary_measurement %>% 
  left_join(summary.measurement.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x)

merged_summary_of_observations_areashape <- summary_observations %>% 
  left_join(summary.observations.areashape.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x)

merged_summary_of_observations_ch2 <- summary_observations %>% 
  left_join(summary.observations.ch2.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x)

merged_summary_of_observations_ch3 <- summary_observations %>% 
  left_join(summary.observations.ch3.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x) 

merged_summary_of_observations_ch4 <- summary_observations %>% 
  left_join(summary.observations.ch4.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x)

merged_summary_of_observations_ch5 <- summary_observations %>% 
  left_join(summary.observations.ch5.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x) 

merged_summary_of_observations_ch6 <- summary_observations %>% 
  left_join(summary.observations.ch6.downsampling, by=c("id_barcode", "id_measurement")) %>% 
  mutate(match=n.x==n.y, ratio=n.y/n.x) 

toc()
```
```{r}
merged_all <- merged_summary_of_measurements %>% select(id_measurement) %>% 
  left_join(merged_summary_of_observations_areashape %>% select(id_measurement, ratio) %>% rename("areashape"="ratio")) %>% 
  left_join(merged_summary_of_observations_ch2 %>% select(id_measurement, ratio) %>% rename("ch2"="ratio")) %>% 
  left_join(merged_summary_of_observations_ch3 %>% select(id_measurement, ratio) %>% rename("ch3"="ratio")) %>% 
  left_join(merged_summary_of_observations_ch4 %>% select(id_measurement, ratio) %>% rename("ch4"="ratio")) %>% 
  left_join(merged_summary_of_observations_ch5 %>% select(id_measurement, ratio) %>% rename("ch5"="ratio")) %>% 
  left_join(merged_summary_of_observations_ch6 %>% select(id_measurement, ratio) %>% rename("ch6"="ratio")) 

numeric_columns <- c("areashape", "ch2", "ch3", "ch4", "ch5", "ch6")

```
```{r}

sweep(merged_all[numeric_columns], 2, colSums(merged_all[numeric_columns]), FUN="/")
m <- scale(merged_all[numeric_columns], center=FALSE, scale=colSums(merged_all[numeric_columns]))


# library(reshape2)
# 
# mtcars.long <- melt(merged_all, id = "id_measurement", measure = numeric_columns)
# 
# ggplot(mtcars.long, aes(id_measurement, value, colour = variable)) + geom_line()
```
...
cell.morphology.all.resolution2 <- m.ds %>% 
  merge(o.ds.a) %>% 
  merge(o.ds.ch2 %>% filter(downsampling=="resolution2") %>% select(-downsampling)) %>% 
  merge(o.ds.ch3 %>% filter(downsampling=="mid24") %>% select(-downsampling)) %>%
  merge(o.ds.ch4 %>% filter(downsampling=="mid24") %>% select(-downsampling)) %>%
  merge(o.ds.ch5 %>% filter(downsampling=="mid24") %>% select(-downsampling)) %>% 
  merge(o.ds.ch6 %>% filter(downsampling=="mid24") %>% select(-downsampling))
...
barcode_id <- "000012117703"

experiment_object_data_ds <- measurement.downsampling %>% 
  filter(id_barcode==barcode_id) %>% 
  select(id_observation, measurement_no, iteration_no, well, field) %>% 
  merge(observation.downsampling.areashape %>% select(id_observation, image_number, object_number), by="id_observation" ) %>% 
  collect()
...
experiment_object_data_ds_v2 <- experiment_object_data_ds %>% 
  mutate(column=as.numeric(substr(well,2,3))) %>%
  mutate(row=substr(well,1,1))
...
```{r}
g <- experiment_object_data_ds_v2 %>% 
  group_by(measurement_no, iteration_no, row, column) %>%
  summarize(n=n()) %>%
  ggplot( aes(x=column,y=row,fill=n) ) +
    geom_tile() +
...
t <- Sys.time()
ggsave(paste0(barcode_id, "_ds_", t, "_object_count.pdf"), plot=g, path="~/mcsaba/biosensor/logs/", dpi=300, bg="white")
```
...
measurement_id_to_truncate <- "000012116403__2022-09-29T12_57_04-Measurement_4"

# deleteQuery <- glue::glue_sql("DELETE FROM measurement WHERE id_measurement LIKE '%000012117203__2023-03-14T12_32_09-Measurement_2%'", .con=pool.v3.ds)
# dbExecute(statement = deleteQuery, con=pool.v3.ds)

deleteQuery <- glue::glue_sql("DELETE FROM measurement WHERE id_measurement LIKE {measurement_id_to_truncate}", .con=con.v3.ds)
print(deleteQuery)
dbExecute(statement = deleteQuery, con=con.v3.ds)

toc()

# for (feature_group in c("areashape", "ch2", "ch3", "ch4", "ch5", "ch6")) {
#   tic()
#   deleteQuery <- gsub("'","",glue::glue_sql("DELETE FROM observation_{feature_group} WHERE id_observation LIKE \"{measurement_id_to_truncate}\"", .con=con.v3.ds))
#   print(deleteQuery)
#   # dbExecute(statement = deleteQuery, con=con.v3.ds)
#   toc()
# }
...
tic()
deleteQuery <- glue::glue_sql("DELETE FROM observation_areashape WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
deleteQuery <- glue::glue_sql("DELETE FROM observation_ch2 WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
deleteQuery <- glue::glue_sql("DELETE FROM observation_ch3 WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
deleteQuery <- glue::glue_sql("DELETE FROM observation_ch4 WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
deleteQuery <- glue::glue_sql("DELETE FROM observation_ch5 WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
deleteQuery <- glue::glue_sql("DELETE FROM observation_ch6 WHERE id_observation LIKE '%000012116403__2022-09-29T12_57_04-Measurement_4%'", .con=pool.v3.ds)
dbExecute(statement = deleteQuery, con=pool.v3.ds)
toc()
```
