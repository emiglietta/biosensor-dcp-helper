...
# results_path_base= paste0("~/bucket/flatfield/", plate_name,"/") #deprecated: only used for result collection
...
## Parsing Index.idx.xml file with

loaddata_csv_file <- paste0(new_path_base, "loaddata_output.csv")
bash_file_for_parsing <- paste0(new_path_base, "pe2loaddata.sh")
fileConn<-file(bash_file_for_parsing)

c('#!/bin/sh',
  'pip install --user pyyaml ipython', #ugly way of managing the dependency of the cellprofiler function
  paste('python',
        '~/mcsaba/biosensor/src/dcp_helper/python/pe2loaddata.py',
        paste0('--index-directory=', inbox_path_base),
...
system(paste0("chmod +x ", bash_file_for_parsing))
system(bash_file_for_parsing)
...
#     channel_of_interest = channel_metadata_regexp[1], #brightfield
#     name = "pc",
...
#
# tic()
# for(i in 2:length(channel_abbreviation)){
#   print(paste0("creating ", channel_abbreviation[i], " metadata"))
#   for(j in 1:length(inbox_path_base)){
#     metadata_split_path <- create_flatfield_metadata_split(
#       path = inbox_path_base[j],
#       channel_of_interest = channel_metadata_regexp[i], #brightfield
#       name = channel_abbreviation[i],
#       json_path = new_json_path, #not needed
#       path_base = new_path_base[j],
#       force = FALSE)
#   }
...
#   path <- generate_group(plate_name[i], channel_abbreviation, new_path_base[i])
#   print(path)
...
#
# tic()
# channel_abbreviation_mod <- channel_abbreviation[3:4]
# for(j in new_path_base){
#   for(i in 1:length(channel_abbreviation_mod)){
#     link_json_metadata(metadata_split_path = list.files(j, pattern = "metadata_", full.names = TRUE) %>%
#                          stringr::str_subset(pattern = ".csv") %>%
#                          stringr::str_subset(pattern = channel_abbreviation_mod[i]),
...
# tic()
# for(j in new_path_base){
#   for(i in 1:length(channel_abbreviation)){
